...
class MyProfileController extends BaseController
{
    ...
    {
        $setTitle = __('file.My Profile');
        $this->setPageData($setTitle,$setTitle,'far fa-id-badge');
        ...
        if($request->ajax())
        {
            if(auth()->check())
            {
                ...
                if (!Hash::check($request->current_password, $user->password)) {
                    $output = ['status'=>'error','message'=>$this->responseMessage('Current Password')];
                }else{
                    $user->password = $request->password;
                    if($user->save()){
                        $output = ['status'=>'success','message'=>$this->responseMessage('Changed Password')];
                    }else{
                        $output = ['status'=>'error','message'=>$this->responseMessage('Failed Password')];
                    }
                }
                return response()->json($output);
            }
