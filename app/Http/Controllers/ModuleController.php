    ...
    public function storeOrUpdate(ModuleFormRequest $request)
    {
        if($request->ajax()){
            if(permission('menu-module-add') || permission('menu-module-edit')){
                ...
                $output       = $result ? ['status' => 'success','message' => !empty($request->update_id) ? 'Data Has Been Updated Successfully' : 'Data Has Been Saved Successfully'] : ['status' => 'error','message' => !empty($request->update_id) ? 'Failed To Update Data' : 'Failed To Save Data'];
