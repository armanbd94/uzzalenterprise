...
class CustomerGroupController extends BaseController
{
    ...
    public function index()
    {
        if (permission('customer-group-access')){
            $setTitle = __('file.Settings');
            $setSubTitle = __('file.Customer Group');
            $this->setPageData($setSubTitle,$setSubTitle,'fas fa-users',[['name' => $setTitle],['name'=>$setSubTitle]]);
        ...
        if($request->ajax()){
            ...
            foreach ($list as $value) {
                ...
                if(permission('customer-group-delete')){
                    $action .= ' <a class="dropdown-item delete_data"  data-id="' . $value->id . '" data-name="' . $value->group_name . '">'.$this->actionButton('Delete').'</a>';
                }
