          ...
          @csrf
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <x-form.selectbox labelName="{{__('file.Delivery Status')}}" name="delivery_status" required="required"  col="col-md-12" class="selectpicker">
                      @foreach (DELIVERY_STATUS as $key => $value)
                      <option value="{{ $key }}">{{ $value }}</option>
                      @endforeach
                    </x-form.selectbox>
                    <x-form.textbox labelName="{{__('file.Delivery Date')}}" name="delivery_date" required="required" class="date" col="col-md-12"/>
                </div>
            </div>
            <!-- /modal body -->
            <!-- Modal Footer -->
            ...
            <button type="button" class="btn btn-primary btn-sm" id="delivery-save-btn">{{__('file.Save')}}</button>
            </div>
            <!-- /modal footer -->
        </form>
      </div>
