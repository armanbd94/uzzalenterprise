                ...
                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form action="" id="sale_update_form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <x-form.textbox labelName="{{__('file.Sale Date')}}" name="sale_date" value="{{ $sale->sale_date }}" required="required" class="date" col="col-md-3"/>
                                            ...
                                            <td colspan="4">
                                                <div class="row">
                                                    <x-form.selectbox labelName="{{ __('file.Payment Status') }}" name="payment_status" required="required"  col="col-md-6 mb-0" class="fcs selectpicker" data-live-search="true">
                                                        ...
                                                        <option value="{{ $key }}" {{ $key == $sale->payment_status ? 'selected' : '' }}>{{ $value }}</option>
                                                        @endforeach
                                                    </x-form.selectbox>
                                                    <x-form.selectbox labelName="{{ __('file.Payment Method') }}" name="payment_method" onchange="account_list(this.value)" required="required"  col="col-md-6 payment_row {{ $sale->payment_status != 3 ? '' : 'd-none' }}" class="selectpicker" data-live-search="true">
                                                        @foreach (SALE_PAYMENT_METHOD as $key => $value)
                                                        <option value="{{ $key }}" {{ $sale->payment_method == $key ? 'selected' : '' }}>{{ $value }}</option>
                                                        @endforeach
                                                    </x-form.selectbox>
                                                </div>
                                            </td>
                                            <td class="text-right font-weight-bolder">{{ __('file.Grand Total') }}</td>
                                            <td><input type="text" class="fcs form-control text-right bg-secondary" value="{{ $sale->grand_total }}" name="grand_total" id="grand_total" placeholder="0.00" readonly></td>
                                            <td></td>
                                        </tr>
                                        <tr>
                                            <td colspan="4">
                                                <div class="row payment_row {{ $sale->payment_status != 3 ? '' : 'd-none' }}">
                                                    ...
                                                    <div class="form-group col-md-6 {{ $sale->payment_method != 1 ? '': 'd-none'}} reference_no">
                                                        ...
                                                        <input type="text" class="fcs form-control" value="{{ $sale->reference_no }}" name="reference_no" id="reference_no">
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <input type="text" class="fcs form-control text-right" value="{{ $sale->paid_amount }}" name="paid_amount" id="paid_amount" onkeyup="calculateNetTotal()" placeholder="0.00">
                                                </div>
                                            </td>
                                            <td></td>
                                        </tr>
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="hidden" name="total_cost" id="total_cost" value="{{ $sale->total_cost }}">
                            ...
                            <div class="form-grou col-md-12 text-center pt-5">
                                <a href="{{ route('sale') }}" class="btn btn-danger btn-sm mr-3"><i class="fas fa-window-close"></i>{{__('file.Cancel')}}</a>
                                <button type="button" class="btn btn-primary btn-sm mr-3" id="sale-save-btn" onclick="store_data()"><i class="fas fa-save"></i>{{__('file.Update')}}</button>
                            </div>
                        </div>
                    </form>
                </div>
...
$(document).ready(function () {
    ...
    var count = 1;
    @if(!$sale->hasManyProducts->isEmpty())
    count = "{{ count($sale->hasManyProducts) + 1 }}";
    @endif
...
customer_list("{{ $sale->customer_id }}");
function customer_list(customer_id = '') {
    $.ajax({
        ...
        data: { _token: _token},
        success: function(data) {
            $('#sale_update_form #customer_id').empty().html(data);
            $('#sale_update_form #customer_id.selectpicker').selectpicker('refresh');
            if(customer_id)
            {
                $('#sale_update_form #customer_id').val(customer_id);
                $('#sale_update_form #customer_id.selectpicker').selectpicker('refresh');
...
function store_data(){
    ...
    if (rownumber < 0) {
        ...
        $.ajax({
            ...
            cache: false,
            beforeSend: function(){
                $('#sale-save-btn').addClass('spinner spinner-white spinner-right');
            },
            complete: function(){
                $('#sale-save-btn').removeClass('spinner spinner-white spinner-right');
            },
            success: function (data) {
