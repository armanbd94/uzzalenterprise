                    ...
                    <form id="journal-voucher-form" method="post">
                        ...
                        <div class="row">
                                ...
                                <div class="col-md-12">
                                    <table class="table table-bordered" id="debit-voucher-table">
                                        <thead class="bg-primary">
                                            <th width="40%">{{__('file.Account Name')}}</th>
                                            <th width="25%" class="text-right">{{__('file.Debit')}}</th>
                                            <th width="25%" class="text-right">{{__('file.Credit')}}</th>
                                            <th width="10%"></th>
                                        </thead>
                                        <tbody>
                                            @php
                                            ...
                                            @if (!$voucher->isEmpty())
                                                ...
                                                <tr>
                                                    <td>
                                                        <select name="journal_account[{{ $key + 1 }}][id]" id="journal_account_{{ $key + 1 }}_id" class="form-control selectpicker">
                                                            <option value="">Select Please</option>
                                                            @if (!$transactional_accounts->isEmpty())
                                                            @foreach ($transactional_accounts as $coa)
                                                                <option value="{{ $coa->id }}" {{ ($contrav->chart_of_account_id == $coa->id) ? 'selected' : '' }}>{{ $coa->name }} @if($coa->supplier_id) {{ '(Supplier)' }}  @endif @if($coa->customer_id) {{ '(Customer)' }}  @endif</option>
                                                            @endforeach
                                                            @endif
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <input type="text" class="form-control text-right debit_amount" value="{{ $contrav->debit }}" onkeyup="calculate_total(1)" name="journal_account[{{ $key + 1 }}][debit_amount]" id="journal_account_{{ $key + 1 }}_debit_amount" placeholder="0.00">
                                                    </td>
                                                    <td>
                                                        <input type="text" class="form-control text-right credit_amount" value="{{ $contrav->credit }}" onkeyup="calculate_total(2)" name="journal_account[{{ $key + 1 }}][credit_amount]" id="journal_account_{{ $key + 1 }}_credit_amount" placeholder="0.00">
                                                    </td>
                                                    @if ($key == 0)
                                                        <td></td>
                                                    @else
                                ...
                                <div class="form-group col-md-12 pt-5 text-center">
                                    <a href="{{ url('voucher-approval') }}" type="button" class="btn btn-danger btn-sm mr-3"><i class="far fa-window-close"></i> {{__('file.Cancel')}}</a>
                                    <button type="button" class="btn btn-primary btn-sm mr-3" id="save-btn" onclick="store_data()"><i class="fas fa-sync-alt"></i> {{__('file.Update')}}</button>
                                </div>
                        </div>
                    </form>
                </div>
...
function store_data(){
    ...
    let url = "{{url('journal-voucher/update')}}";
    $.ajax({
        url: url,
        type: "POST",
        data: formData,
