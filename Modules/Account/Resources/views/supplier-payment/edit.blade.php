                    ...
                    <form id="supplier-payment-form" method="post">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <x-form.selectbox labelName="{{__('file.Supplier')}}" name="supplier_id"  onchange="dueAmount(this.value)" required="required"  col="col-md-6" class="selectpicker">
                                    @if (!$suppliers->isEmpty())
                                        @foreach ($suppliers as $supplier)
                                            <option value="{{ $supplier->id }}" {{ $voucher_data[0]->chart_of_account_id == $supplier->coa->id ? 'selected' : '' }}>{{ $supplier->name.' - '.$supplier->mobile }}</option>
                                        @endforeach
                                    @endif
                                </x-form.selectbox>
                                ...
                                <x-form.textbox labelName="{{__('file.Amount')}}" name="amount" required="required" col="col-md-6" value="{{ $voucher_data[0]->debit }}" />
...
function store_data(){
    ...
    let url = "{{url('supplier-payment/update')}}";
    $.ajax({
        url: url,
        type: "POST",
        data: formData,
        ...
        success: function (data) {
            ...
            if (data.status == false) {
                ...
                if (data.status == 'success' && data.supplier_transaction != '') {
                    ...
                    window.location.replace("{{ url('supplier-payment') }}");
