                ...
                <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <form id="debit-voucher-form" method="post">
                        ...
                        <div class="row">
                                ...
                                <x-form.selectbox labelName="{{__('file.Credit Account Head')}}" name="credit_account_id" required="required"  col="col-md-3" class="selectpicker">
                                    @if (!$accounts->isEmpty())
                                ...
                                <div class="col-md-12">
                                    <table class="table table-bordered" id="debit-voucher-table">
                                        <thead class="bg-primary">
                                            <th width="60%">{{__('file.Account Name')}}</th>
                                            <th width="30%" class="text-right">{{__('file.Amount')}}</th>
                                            <th width="10%"></th>
                                        </thead>
                                        <tbody>
                                            @php
                                                    ...
                                                    <td>
                                                        <select name="debit_account[{{ $key + 1 }}][id]" id="debit_account_{{ $key + 1 }}_id" class="form-control selectpicker">
                                                            ...
                                                            @foreach ($transactional_accounts as $coa)
                                                                <option value="{{ $coa->id }}" {{ ($dvoucher->chart_of_account_id ==  $coa->id) ? 'selected' : '' }}>{{ $coa->name }} @if($coa->supplier_id) {{ '(Supplier)' }}  @endif @if($coa->customer_id) {{ '(Customer)' }}  @endif</option>
                                                            @endforeach
                                                            @endif
                                                        </select>
                                                    </td>
