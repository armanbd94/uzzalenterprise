...
<h4 class="text-dark text-center py-3"><b> {{__('file.Trail Balance')}} {{ ($with_details == 1) ? __('file.With Opening') : '' }} ({{__('file.From')}} {{ date('d-M-Y',strtotime($start_date)) }} {{__('file.To')}} {{ date('d-M-Y',strtotime($end_date)) }})</b></h4>

<table class="table table-bordered" style="width:100%;">
    <thead class="bg-primary">
        <tr>
            <th class="text-center">{{__('file.Code')}}</th>
            <th class="text-center">{{__('file.Account Name')}}</th>
        ...
        @if ($with_details == 1)
            ...
            @if($profit_loss != 0)
                <tr class="bg-success text-white font-weight-bolder">
                    <td align="left">&nbsp;</td>
                    <td align="left">{{__('file.Profit-Loss')}}</td>
                    @if($profit_loss < 0)
    ...
    <tfoot>
        <tr class="bg-primary">
            <td colspan="2" class="text-right text-white font-weight-bolder">{{__('file.Total')}}</td>
