                    ...
                    <div class="col-md-2 mb-5">
                      <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                        <a class="nav-link active" id="v-pills-general-tab" data-toggle="pill" href="#v-pills-general" role="tab" aria-controls="v-pills-general" aria-selected="true">
                            <i class="far fa-id-badge mr-2"></i> {{__('file.Update Profile')}}
                        </a>
                        <a class="nav-link" id="v-pills-mail-tab" data-toggle="pill" href="#v-pills-mail" role="tab" aria-controls="v-pills-mail" aria-selected="false">
                            <i class="fas fa-key mr-2"></i> {{__('file.Change Password')}}
                        </a>
                      </div>
                    </div>
                    <div class="col-md-10">
                      <div class="tab-content" id="v-pills-tabContent">
                        <div class="tab-pane fade show active" id="v-pills-general" role="tabpanel" aria-labelledby="v-pills-general-tab">
                            <form id="profile-form" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    <div class="col-md-9">
                                        <div class="row">
                                            <x-form.textbox labelName="{{__('file.Name')}}" name="name" value="{{ Auth::user()->name }}" required="required" col="col-md-6" placeholder="Enter name"/>
                                                <div class="form-group col-md-6">
                                                    <label for="">{{__('file.Username')}}</label>
                                                    <input type="text" class="form-control" value="{{ Auth::user()->username }}" readonly/>
                                                </div>
                                                <x-form.textbox labelName="{{__('file.Phone No')}}." name="phone" value="{{ Auth::user()->phone }}" required="required" col="col-md-6" placeholder="Enter phone number"/>
                                                <x-form.textbox labelName="{{__('file.Email')}}" type="email" name="email" value="{{ Auth::user()->email }}" col="col-md-6" placeholder="Enter email"/>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group col-md-12">
                                            <label for="">{{__('file.Profile Photo')}}</label>
                                    ...
                                    <div class="form-group col-md-12" style="padding-top:20px;">
                                        <button type="button" class="btn btn-primary btn-sm" id="save-profile" onclick="save_data('profile')"><i class="far fa-save"></i> {{__('file.Save')}}</button>
                                    </div>
                                </div>
                        ...
                        <div class="tab-pane fade" id="v-pills-mail" role="tabpanel" aria-labelledby="v-pills-mail-tab">
                            <p class="text-danger">**({{__('file.Password length minimum 8 characters and maximum 30 characters')}})</p>
                            <form id="password-form" method="POST">
                                @csrf
                                <div class="col-md-6 form-group required">
                                    <label for="site_title">{{__('file.Current Password')}}</label>
                                ...
                                <div class="col-md-6 form-group required">
                                    <label for="site_title">{{__('file.New Password')}}</label>
                                    <div class="input-group">
                                        <input type="password" class="form-control bg-brand" name="password" id="password">
                                        <div class="input-group-prepend">
                                            <span class="input-group-text bg-warning" id="generate_password" style="border:0;cursor: pointer;" data-toggle="tooltip" data-theme="dark" title="{{__('file.Generate Password')}}">
                                ...
                                <div class="form-group col-md-12" style="padding-top:20px;">
                                    <button type="button" class="btn btn-primary btn-sm" id="save-password" onclick="save_data('password')"><i class="far fa-save"></i> {{__('file.Save')}}</button>
                                </div>
                            </form>
                        </div>
                      </div>
...
$(document).ready(function(){
    ...
    $("#avatar").spartanMultiImagePicker({
        ...
        onExtensionErr : function(index, file){
            Swal.fire({icon: 'error',title: 'Oops...',text: '{{__('file.Only png,jpg,jpeg,gif file format allowed!')}}'});
        },
    });
