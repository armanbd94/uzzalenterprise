            ...
            <ul class="menu-nav ">
                @foreach (Session::get('user_menu') as $menu)
                    @if($menu->children->isEmpty())
                        @php
                            $menu_title =  __('file.').$menu->module_name;
                            $divider_title =  __('file.').$menu->divider_title;
                        @endphp
                        @if ($menu->type == 1)
                            <li class="menu-section ">
                                <h4 class="menu-text">{{ __($divider_title) }}</h4>
                            </li>
                        @else
                            <li class="menu-item  {{ (request()->is($menu->url)) ? 'menu-item-active' : '' }}" aria-haspopup="true">
                                <a href="{{ $menu->url ? url($menu->url) : '' }}" class="menu-link" target="{{ $menu->target ?? '_self' }}">
                                    ...
                                    <span class="menu-text">{{ __($menu_title) }}</span>
                                </a>
                            </li>
                        @endif
                    @else
                        @php
                            $menu_second_title =  __('file.').$menu->module_name;
                        @endphp
                        ...
                        " aria-haspopup="true" data-menu-toggle="hover">
                            <a href="javascript:void(0);" class="menu-link menu-toggle">
                                <span class="svg-icon menu-icon"><i class="{{ $menu->icon_class }}"></i></span>
                                <span class="menu-text">{{ __($menu_second_title) }}</span>
                            ...
                            <div class="menu-submenu ">
                                ...
                                <ul class="menu-subnav">
                                    @foreach ($menu->children as $submenu)
                                        @if($submenu->children->isEmpty())
                                            @php
                                                $sub_menu_title =  'file.'.$submenu->module_name;
                                            @endphp
                                            <li class="menu-item {{ (request()->is($submenu->url)) ? 'menu-item-active' : '' }}" aria-haspopup="true">
                                                <a href="{{ $submenu->url ? url($submenu->url) : '' }}" class="menu-link ">
                                                    ...
                                                    <span class="menu-text">{{ __($sub_menu_title) }}</span>
                                                </a>
                                            </li>
                                        @else
                                            @php
                                                $sub_menu_second_title =  __('file.').$submenu->module_name;
                                            @endphp
                                        <!-- -->
                                            <li class="menu-item  menu-item-submenu
                                                @foreach ($submenu->children as $sub_submenu)
                                                ...
                                                <a href="javascript:void(0);" class="menu-link menu-toggle">
                                                    <i class="menu-bullet menu-bullet-dot"><span></span></i>
                                                    <span class="menu-text">{{ __($sub_menu_second_title) }}</span>
{{--                                                    <span class="menu-text">{{ $submenu->module_name }}</span>--}}
                                                ...
                                                <div class="menu-submenu ">
                                                    ...
                                                    <ul class="menu-subnav">
                                                        @foreach ($submenu->children as $sub_submenu)
                                                            @php
                                                                $sub_menu_third_title =  'file.'.$sub_submenu->module_name;
                                                            @endphp
...
{{--                                                                <span class="menu-text">{{ $sub_submenu->module_name }}</span>--}}
                                                                <span class="menu-text">{{ __($sub_menu_third_title) }}</span>
                                                            </a>
                                                        </li>
                                                        @endforeach
                                                    </ul>
